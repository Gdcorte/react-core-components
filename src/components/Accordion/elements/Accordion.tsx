...
const AccordionContainer = styled.div`
  color: ${({ theme: { theme } }) =>
    FontHelper.findBestContrast(theme.background.base, [
      theme.fonts.dark,
      theme.fonts.light,
    ])};
...
const AccordionItem: FunctionComponent<AccordionMenuProps> = ({
  ...
  return (
    <AccordionContainer
      className={`accordionContainer ${option.uniqueKey} ${
        option.commonClass || ""
      } container-lvl-${currentLevel}`}
    >
      <AccordionTitle
        key={option.uniqueKey}
        onClick={changeMenu}
      ...
      {openMenu == option.uniqueKey && (
        <AccordionBody
          className={`accordionBody body-lvl-${currentLevel} ${
            option.bodyClass || ""
          }`}
        >
          {renderedOptions}
